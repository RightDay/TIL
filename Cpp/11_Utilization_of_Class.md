...
C++가 내장 데이터형을 변환할 때에는, 표준 데이터형의 어떤 값을 다른 표준 데이터형의 한 변수에 대입하는 구문을 작성하면, 두 데이터형이 서로 호환될 때, C++는 그 **값의 데이터형을 대입받는 입장에 있는 변수와 동일한 데이터형으로 자동으로 변환한다.**
...
다양한 수치 데이터형들이 모두 기본적으로 동일한 수(number)를 나타내고, C++가 데이터형 변환을 위한 내부 규칙을 적용하기 때문에, 이러한 대입 구문들은 바르게 동작한다.

그러나 이러한 변환 과정에서 정확성에 손실을 입을 수 있다.

C++는 **호환되지 않는 데이터형은 자동으로 변환되지 않는다.**
...
```cpp
int * p = (int *) 10;	// p와 (int *) 10이 모두 포인터이므로 OK
```
