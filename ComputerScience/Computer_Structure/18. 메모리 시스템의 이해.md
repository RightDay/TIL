...
![memory_system](./images/memory_system.png)

## 메모리 계층

기억장치는 CPU에 의해 시행될 프로그램이 저장되는 곳이다.
...
- 주 기억장치에 접근하는 시간은 프로세스의 논리회로의 속도보다 느리기 때문에 **자주 사용하는 프로그램을 사용할 때 비효율적이다.** 때문에 **사용빈도가 높은 부분은 캐쉬 메모리에 넣어 놓아 효율적인 작업수행을 가능**하게 한다. MMS와 CPU에 배치해서 사용한다.
...
- Magnetic Tape, USB, SD card - 필요한 파일의 생성 및 삭제가 가능하다.
...
![computer_system_memory](./images/computer_system_memory.png)
...
![Auxiliary_Memory_Unit](./images/Auxiliary_Memory_Unit.png)
